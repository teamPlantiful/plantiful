...
export default function PlantListSection() {
  ...
  const sortedPlants = useMemo(() => {
    ...
    // 물주기 우선 정렬
    return [...plants].sort((a, b) => {
      const ddayA = a.next_watering_date
        ? Math.floor(
            (new Date(a.next_watering_date).getTime() - Date.now()) / (1000 * 60 * 60 * 24)
          )
        : Infinity
      const ddayB = b.next_watering_date
        ? Math.floor(
            (new Date(b.next_watering_date).getTime() - Date.now()) / (1000 * 60 * 60 * 24)
          )
        : Infinity
      return ddayA - ddayB
    })
    ...
    <>
      ...
      {sortedPlants.length === 0 ? (
        ...
        <section className="grid gap-3 grid-cols-1 md:grid-cols-2">
          {sortedPlants.map((p) => {
            const ddayWater = p.next_watering_date
              ? Math.floor(
                  (new Date(p.next_watering_date).getTime() - Date.now()) / (1000 * 60 * 60 * 24)
                )
              : 0
